...
const CommentCard = (props) => {
  ...
  return (
    <>
      <div key={props.comment.id} className="comment-card bg-white dark:bg-slate-900 dark:text-stone-200 rounded-md p-3 lg:flex lg:justify-start w-full lg:flex-row-reverse lg:p-5">
        <div className='w-full'>
          <div className="profile-container flex justify-start items-center gap-3 w-full">
            ...
            <strong className="username  font-semibold tracking-wide">{props.comment.user.username}</strong>
          ...
          <div className="comment-body w-full py-4">
            {
              !editing && <p className='text-grayish-Blue dark:text-stone-300'>
                <strong className='text-blue-800 tracking-wide'>{props.comment.replyingTo ? [`@ ${props.comment.replyingTo} `] : ''}</strong>
            ...
            }
            {
              editing && <div className='overflow-auto'>
                <textarea className='w-full dark:bg-slate-800 p-2 dark:caret-red-500' name="textarea" id="textarea" rows="3" value={value} onChange={(e) => { setValue(e.target.value) }}></textarea>
                <button className=" sendBtn float-right bg-modarate-blue text-white font-semibold px-3 py-1 mt-2 rounded-md" onClick={() => { props.updateComment(value, (props.parentId ? props.parentId : null), props.comment.id); setEditing(false) }}>UPDATE</button>
              </div>
            }
          </div>
        </div>
        <div className="upvote-and-replay-container flex justify-between items-center lg:items-start lg:mr-5">
          <div className="upvote flex items-center space-x-3 bg-very-light-gray dark:bg-slate-800 py-2 px-3 rounded-xl lg:flex-col lg:space-x-0 lg:space-y-3 lg:py-5 ">
            ...
            <p className='text-lg font-semibold text-blue-800 dark:text-stone-300'>{props.comment.score}</p>
